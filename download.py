...
for key, value in GOOGLE_BOOKS_VOLUMES.items():
    ...
    if os.path.exists(pdf_path):
        print(f'File {pdf_path} already exists')
        continue
    r = requests.get(f'https://www.googleapis.com/books/v1/volumes/{value}?key={os.getenv("GOOGLE_API_KEY")}')
    ...
    capid_image_array = np.asarray(bytearray(requests.get(capid_image_url).content), dtype=np.uint8)
